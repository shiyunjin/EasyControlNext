...
public class DbHelper extends SQLiteOpenHelper {
  ...
  public void onCreate(SQLiteDatabase db) {
    ...
    stringBuilder.append("pairPort integer,");
    stringBuilder.append("pairKey text);");
  ...
  private ContentValues getValues(Device device) {
    ...
    values.put("pairPort", device.pairPort);
    values.put("pairKey", device.pairKey);
    return values;
  }
  ...
  private Device getDeviceFormCursor(Cursor cursor) {
    ...
    for (int i = 0; i < cursor.getColumnCount(); i++) {
      switch (cursor.getColumnName(i)) {
        ...
        case "pairPort": {
          device.pairPort = cursor.getInt(i);
          break;
        }
        case "pairKey": {
          device.pairKey = cursor.getString(i);
          break;
        }
      }
    }
