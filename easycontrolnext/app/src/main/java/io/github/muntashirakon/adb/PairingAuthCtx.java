...
class PairingAuthCtx implements Destroyable {
    ...
    private static final byte[] CLIENT_NAME = io.github.muntashirakon.adb.StringCompat.getBytes("adb pair client\u0000", "UTF-8");
    private static final byte[] SERVER_NAME = io.github.muntashirakon.adb.StringCompat.getBytes("adb pair server\u0000", "UTF-8");

    // The following values are taken from the following source and are subjected to change
    // https://github.com/aosp-mirror/platform_system_core/blob/android-11.0.0_r1/adb/pairing_auth/aes_128_gcm.cpp
    private static final byte[] INFO = io.github.muntashirakon.adb.StringCompat.getBytes("adb pairing_auth aes-128-gcm key", "UTF-8");
