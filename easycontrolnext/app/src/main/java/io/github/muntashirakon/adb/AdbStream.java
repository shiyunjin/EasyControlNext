...
public class AdbStream implements Closeable {
    ...
    void sendReady() throws IOException {
        ...
        mAdbConnection.sendPacket(io.github.muntashirakon.adb.AdbProtocol.generateReady(mLocalId, mRemoteId));
    }
    ...
    public void write(byte[] bytes, int offset, int length) throws IOException {
        ...
        while (length != 0) {
            if (length <= maxData) {
                mAdbConnection.sendPacket(io.github.muntashirakon.adb.AdbProtocol.generateWrite(mLocalId, mRemoteId, bytes, offset, length));
                offset = offset + length;
                length = 0;
            } else { // if (length > maxData) {
                mAdbConnection.sendPacket(io.github.muntashirakon.adb.AdbProtocol.generateWrite(mLocalId, mRemoteId, bytes, offset, maxData));
    ...
    public void close() throws IOException {
        ...
        mAdbConnection.sendPacket(io.github.muntashirakon.adb.AdbProtocol.generateClose(mLocalId, mRemoteId));
    }
